...
with open("clustering_data_income.csv") as file:
    num_roles = sum(1 for row in file)
    file.close()

with open("clustering_data_income.csv") as csv_file:
    ...
    for row in reader:
        ...
        if row[36] != "NULL":
            rate = float(row[36])
        income_rate = 0
        if row[461] != "NULL" and row[461] != "PrivacySuppressed":
            income_rate = float(row[461])
        state = row[5]
        if rate != 0 and income_rate != 0:
            university_clustering_attributes = (rate, income_rate)
...
for i in range(len(db.labels_)):
    ...
    universities_labels[i].append(str(universities[i][0]))
    universities_labels[i].append(str(universities[i][1]))
    universities_labels[i].append(str(db.labels_[i]))

with open("clustering_result_income.csv", mode='w') as written_file:
